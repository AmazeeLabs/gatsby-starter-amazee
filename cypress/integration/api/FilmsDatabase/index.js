...
Given(/^a visitor requests the list of films with "([^"]*)"$/, function(name) {
  cy.graphqlQuery(`{ allPersons { name, id } }`).then( result => {
    const id = result.body.data.allPersons.filter(p => p.name === name).pop().id;
    cy.graphqlQuery(`query ($id: ID!) { Person(id: $id) { films { title } } }`, {id}).then( result => {
...
Given(/^a visitor requests the list of characters in "([^"]*)"$/, function(title) {
  cy.graphqlQuery(`{ allFilms { title, id } }`).then( result => {
    const id = result.body.data.allFilms.filter(f => f.title === title).pop().id;
    cy.graphqlQuery(`query ($id: ID!) { Film(id: $id) { characters { name } } }`, {id}).then( result => {
